...
export default function ControlePage() {
  ...
  return <div className="app">
    ...
    <main className="content">
      ...
      <section className="panel chartPanel wideChart"><div className="panelHead"><div><h2>Evolução do Fluxo de Caixa do Período</h2><p>Resultado mensal oficial da base importada</p></div><strong className="chartHighlight">{moeda(anual.fluxo)} no ano</strong></div><LineChart values={fluxoMensal} /></section>
      <section className="panel monthlyPanel"><div className="panelHead"><div><h2>Indicadores financeiros</h2><p>Meses sempre na horizontal para comparação direta.</p></div><div className="savingBadge">Taxa anual de poupança: <b>{taxaPoupanca.toFixed(1)}%</b></div></div>
        <div className="monthlyTableWrap"><table className="monthlyTable"><thead><tr><th className="indicatorHeader">Indicador</th>{meses.map(m => <th key={m}>{m}</th>)}<th className="actionsHeader">Ações</th></tr></thead><tbody>
          ...
          <tr className="sectionRow flowRow"><td className="sectionLabel"><BarChart3 size={25} /><strong>Fluxo de Caixa</strong></td>{meses.map((m, i) => <td key={m}>{moeda(valor('Fluxo de caixa', i))}</td>)}<td className="sectionAction">—</td></tr>
          <tr className="sectionRow periodRow"><td className="sectionLabel"><BarChart3 size={25} /><strong>Fluxo de Caixa do Período</strong></td>{meses.map((m, i) => <td key={m}>{moeda(fluxoMensal[i])}</td>)}<td className="sectionAction">—</td></tr>
        ...
        <p className="tableNote">As linhas detalhadas podem ser ajustadas manualmente. Os subtotais de Salários e Recebíveis e Despesas Totais são recalculados automaticamente.</p>
      </section>
