...
export default function ControlePage() {
  ...
  const subtotaisReceita = useMemo(() => tiposReceita.map(tipo => {
    const itens = receitas.filter(r => r.tipo === tipo)
    return { tipo, planejado: itens.reduce((s, r) => s + r.planejado, 0), realizado: itens.reduce((s, r) => s + r.realizado, 0), leo: itens.filter(r => r.responsavel === 'Léo').reduce((s, r) => s + r.realizado, 0), nat: itens.filter(r => r.responsavel === 'Nat').reduce((s, r) => s + r.realizado, 0) }
  }), [receitas])
      ...
      <main className="content">
        ...
        {modulo === 'receitas' ? (
          <>
            ...
            <section className="grid moduleBottom"><article className="panel"><div className="panelHead"><div><h2>Resumo de receitas</h2><p>Subtotais por tipo: Léo + Nat.</p></div></div><div className="controlRows receiptSummary">{subtotaisReceita.map(s => <div className="receiptBlock" key={s.tipo}><div className="receiptSubtotal"><div><strong>{s.tipo}</strong><span>Subtotal Léo + Nat</span></div><b>{moeda(s.realizado)}</b></div><div className="receiptPeople"><div><span>Léo</span><b>{moeda(s.leo)}</b></div><div><span>Nat</span><b>{moeda(s.nat)}</b></div></div></div>)}<div className="receiptTotal"><span>Renda Familiar</span><b>{moeda(totaisReceita.realizado)}</b></div></div></article><aside className="controlAside"><article className="panel"><div className="panelHead"><div><h2>Regra de renda</h2><p>Somente a renda é individualizada.</p></div></div><p className="moduleNote">Cada tipo de receita apresenta um subtotal consolidado de Léo + Nat. O total da Renda Familiar soma todos os tipos.</p></article></aside></section>
          </>
        ) : (
          <>
