...
function LineChart({ values }: { values: number[] }) {
  ...
  return <div className="lineChartWrap"><svg viewBox={`0 0 ${width} ${height}`} className="lineChart" role="img" aria-label="Evolução do fluxo de caixa"><line x1={pad} y1={height - pad} x2={width - pad} y2={height - pad} className="axis" /><polyline points={`${pad},${height - pad} ${points} ${width - pad},${height - pad}`} className="areaLine" /><polyline points={points} className="mainLine" />{values.map((v, i) => { const x = pad + i * (width - pad * 2) / (values.length - 1); const y = height - pad - (v - min) / range * (height - pad * 2); return <circle key={i} cx={x} cy={y} r="3.5" className="linePoint" /> })}</svg><div className="lineLabels">{meses.map(m => <span key={m}>{m.split('/')[0]}</span>)}</div></div>
}
...
export default function ControlePage() {
  ...
  const fluxoCaixaMensal = fluxoPeriodoCalculado.reduce<number[]>((acumulado, valorMes) => { acumulado.push((acumulado[acumulado.length - 1] || 0) + valorMes); return acumulado }, [])
  const anual = useMemo(() => ({ renda: receitaMensal.reduce((s, v) => s + v, 0), despesas: despesasMensal.reduce((s, v) => s + v, 0), fluxo: fluxoCaixaMensal[fluxoCaixaMensal.length - 1] || 0 }), [dados])
  ...
  return <div className="app">
    ...
    <main className="content">
      ...
      <section className="panel chartPanel wideChart"><div className="panelHead"><div><h2>Evolução do Fluxo de Caixa</h2><p>Saldo acumulado mês a mês a partir do Fluxo de Caixa do Período</p></div><strong className="chartHighlight">{moeda(anual.fluxo)} acumulado</strong></div><LineChart values={fluxoCaixaMensal} /></section>
